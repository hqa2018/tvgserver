...
myLib.desktop = {
	winWH : function() {
		$('body').data('winWh', {//窗口的大小
			'w' : $(window).width(),
			'h' : window.innerHeight
//			'h' : $(window).height()
		});
	},
	desktopPanel : function() {//初始化面板，即每个DIV层
		$('body').data('panel', {
			'taskBar' : {//任务栏
				'_this' : $('#taskBar'),
				'task_lb' : $('#task_lb')
			},
			'navBar' : $('#navBar'),//顶部页面切换键
			'wallpaper' : $('#wallpapers'),//背景
			'lrBar' : {//左边导航
				'_this' : $('#lr_bar'),
			...
			'powered_by' : $('a.powered_by'),//右下角水印
			'YhNote':$('#YhNote')
		});
	},
	iconDataInit : function(data) {//设置每个图标的数据
		for ( var a in data) {
			$("#" + a).data("iconData", data[a]);
		}
	},
	getMydata : function() {//获取body当前的data
		return $('body').data();
	},
	mouseXY : function() {//获取鼠标当前经纬度
		var mouseXY = [];
...
myLib.desktop.win = {
	winHtml : function(title, url, id) {
		...
		+ id + "><div class='win_title'><b>"
		+ title + "</b><span class='win_btnblock'><a href='#' class='winMinimize' title='最小化'></a><a href='#' class='winMaximize' title='最大化'></a><a href='#' class='winHyimize' title='还原'></a><a href='#' class='winClose' title='关闭'></a></span></div><iframe frameborder='0' name='myFrame_"
		+ id + "' id='myFrame_"
		+ id + "' class='winframe' scrolling='auto' width='100%' src=''></iframe></div>";
		
		/*+ id + "><div class='win_title'><b>"
		+ title + "</b><span class='win_btnblock'><a href='#' class='winClose' title='关闭'></a></span></div><iframe frameborder='0' name='myFrame_"
		+ id + "' id='myFrame_"
		+ id + "' class='winframe' scrolling='auto' width='100%' src=''></iframe></div>";*/
	},
	...
	iframFix : function(obj) {
		obj.each(function() {
			...
			if (o.find('.zzDiv').size() <= 0)
				o.append($("<div class='zzDiv' style='width:100%;height:"
				+ (o.innerHeight() - 26)
				+ "px;position:absolute;z-index:900;left:0;top:26px;'></div>"));
		})
	},
	// 获取当前窗口最大的z-index值
	...
	maximizeWin : function(obj) {
		var myData = myLib.desktop.getMydata(), panel = $("#desktopInnerPanel")
				.offset(), wh = myData.winWh;// 获取当前document宽高
		obj.css({
			'width' : wh['w'],
			'height' : wh['h'],
			'left' : -panel.left,
			'top' : -panel.top
		}).draggable("disable").resizable("disable").fadeTo("fast", 1).find(
				".winframe").css({
			'width' : wh['w'],
			'height' : wh['h'] - 26
		});
	...
	switchZindex : function(obj) {
		var myData = myLib.desktop.getMydata(), $topWin = myData.topWin, $topWinTab = myData.topWinTab, curWinZindex = obj
				.css("z-index"), maxZ = myData.maxZindex, objTab = myLib.desktop.taskBar
				.findWinTab(obj);
	...
	newWin : function(options) {
		...
		var myData = myLib.desktop.getMydata(), wh = myData.winWh, // 获取当前document宽高
		$windows = $("div.windows"), _this = this, curwinNum = myLib._is(
				myData.winNum, "Number") ? myData.winNum : 0;// 判断当前已有多少窗口
		...
		if ($("#" + id).size() <= 0) {
			// 在任务栏里添加tab
			myLib.desktop.taskBar.addWinTab(options['WindowTitle'],
					options['WindowsId']);
			// 初始化新窗口并显示
			$(_this.winHtml(options['WindowTitle'], options['iframSrc'], id))
					.appendTo('#desktopInnerPanel');
			...
			if (options.WindowAnimation == 'none') {
				$newWin.css({
					...
					$(this).find(".winframe").attr("src",options['iframSrc']).load(function() {
						$(this).show();//显示加载页面内容--cb
					});
				});
			} else {
				$newWin.css({
					"left" : $iconOffset.left - 60,
					"top" : $iconOffset.top - 80,
					"z-index" : zindex
				...
				100,
				function() {
					$(this).find(".winframe").attr("src",
							options['iframSrc']).load(function() {
						$(this).show();//显示加载页面内容--cb
					});
				});
			}
			...
			.bind({
				...
				"width" : options['WindowWidth'],
				"height" : options['WindowHeight'] - 26
			});
			
			// 调用窗口拖动,参数可拖动的范围上下左右，窗口id和，浏览器可视窗口大小
			if (options['WindowDraggable']) {
				_this.drag([ 0, 0, wh['w'] - options['WindowWidth'] - 10,
						wh['h'] - options['WindowHeight'] - 35 ], $newWin, wh);
			}
			// 调用窗口resize,传递最大最小宽度和高度，新窗口对象id，浏览器可视窗口大小
			if (options['WindowResizable']) {
				_this.resize(options['WindowMinWidth'],
						options['WindowMinHeight'], wh['w'] - wLeft, wh['h']
								- wTop - 35, $newWin, wh);
			}
			...
			// 如果已存在此窗口，判断是否隐藏
			var wins = $("#" + id), objTab = myLib.desktop.taskBar
					.findWinTab(wins);
			if (wins.is(":hidden")) {
				wins.show();
...
myLib.desktop.lrBar = {
	upLrBar : function() {
		var myData = myLib.desktop.getMydata(), $lrBar = myData.panel.lrBar['_this'], wh = myData.winWh;
		$lrBar.css({
			'top' : Math.floor((wh['h'] - $lrBar.height()) / 2) - 40
		});
//		console.info("$lrBar="+$lrBar.height());
//		console.info("wh[h]="+wh['h'])
	},
	init : function(iconData) {
		// 读取元素对象数据
		var myData = myLib.desktop.getMydata(), $default_tools = myData.panel.lrBar['default_tools'], $def_tools_Btn = $default_tools
				.find('span'), $start_btn = myData.panel.lrBar['start_btn'], $start_block = myData.panel.lrBar.start_block, $start_item = myData.panel.lrBar['start_item'], $default_app = myData.panel.lrBar['default_app'], $lrBar = myData.panel.lrBar['_this'], wh = myData.winWh, _this = this;
		...
		// 默认应用程序区
		$default_app.droppable(
				{
					scope : 'a',
					drop : function(event, ui) {
						var title = ui.draggable.find(".text").text();
						ui.draggable.removeClass("desktop_icon").attr({
							"style" : "",
							"title" : title
						}).find("span").removeClass("icon").end().appendTo(
								$default_app);
			...
			$(this).removeClass('btnOver');
				...
				function() {
					...
					$(this).data("currPanel",
							$("ul.currDesktop").index("ul.deskIcon"));
			...
			drop : function(event, ui) {
				...
				ui.draggable.removeClass("desktop_icon").attr({
					"style" : "",
					"title" : title
				}).find("span").removeClass("icon").end().insertBefore(
						$(this));
				_this.init();
				myLib.desktop.deskIcon.init();
			}
		...
		//配置管理权限
		$('#config_btn').toggle(function() {
			console.info("config_btn");
			$('.config_inc').hide();
			$("#ConfigNote").fadeIn("slow");	
			//curwinNum = myLib._is(myData.winNum, "Number") ? myData.winNum : 0;
		}, function() {
			$('.config_inc').show();
			$("#ConfigNote").fadeOut("slow");
		});
		//显示天气
		$('#onduty_btn').toggle(function() {
			$('#YhonDuty').fadeIn("slow");
		}, function() {
			$('#YhonDuty').fadeOut("slow");
		});
		//显示便签
		$("#them_btn").toggle(function() {	
			$("#YhNote").fadeIn("slow");
		}, function() {
			$("#YhNote").fadeOut("slow");
		});
...
function saveOnDuty(){
	$.ajax({
		type:"get",
		url:"/seismis/destop/saveOnDuty",
		data:{
			user:$("#r_person").val(),
			value:$("#specialdutyButton").val(),
			nub: $("#nub").val(),
		},
		dataType:"json",
		error:function(){
...
myLib.desktop.taskBar = {
	...
	init : function() {
		...
		var $task_lb = myData.panel.taskBar['task_lb'], $task_bar = myData.panel.taskBar['_this'], wh = myData.winWh;
	...
	contextMenu : function(tab, id) {
		...
		var data = [[{
				text : "最大化",
				func : function() {
					$("#myWin_" + tab.data('win')).find(
							'a.winMaximize').trigger('click');
				}
			},{
				text : "最小化",
				func : function() {
					myLib.desktop.win.minimize($("#myWin_"
							+ tab.data('win')));
				}
			} ],
			[ {
				...
				func : function() {
					...
					myLib.desktop.win.closeWin($("#myWin_"
							+ tab.data('win')));
				}
			} ] ];
	...
	addWinTab : function(text, id) {
		var myData = myLib.desktop.getMydata();
		var $task_lb = myData.panel.taskBar['task_lb'], $task_bar = myData.panel.taskBar['_this'], $navBar = myData.panel.navBar, $navTab = $navBar
				.find("a"), tid = "myWinTab_" + id, allTab = $task_lb.find('a'), curTabNum = allTab
				.size(), docHtml = "<a href='#' id='" + tid + "'>" + text
				+ "</a>";
		...
		$task_lb.find('a.selectTab').removeClass('selectTab').addClass(
				'defaultTab');

		$newTab.data('win', id).addClass('selectTab').click(
			function() {
				var win = $("#myWin_" + $(this).data('win')), tabId = this.id, iconId = tabId
						.split("_")[1], desk = $("#" + iconId)
						.parent(), i = desk.index("ul.deskIcon"); // 判断窗口在那个桌面区域
				...
				if (desk.is(".currDesktop")) {
					if (win.is(".hideWin")) {
						// win.show();
						win.css({
							"left" : win.position().left + 10000,
							"visibility" : "visible"
						}).removeClass("hideWin");

						$(this).removeClass('defaultTab').addClass(
								'selectTab');// 当只有一个窗口时
		...
		if (curTabNum > myData.maxHdTabNum - 1) {
			...
			rightBtn.show().find("a").click(function() {
				var pos = $task_lb.position(), h = $task_lb
						.height(), row = h / 40;
				if (pos.top > (row - 1) * (-40)) {
					$task_lb.animate({
						"top" : pos.top - 40
	...
	delWinTab : function(wObj) {
		var myData = myLib.desktop.getMydata(), $task_lb = myData.panel.taskBar['task_lb'], $task_bar = myData.panel.taskBar['_this'], LeftBtn = $('#leftBtn'), rightBtn = $('#rightBtn'), pos = $task_lb
				.position();
...
myLib.desktop.navBar = {
	init : function() {
		...
		$navTab.droppable({
			...
			drop : function(event, ui) {
				var i = $navTab.index($(this));
				ui.draggable.addClass("desktop_icon").insertBefore(
						$deskIcon.eq(i).find(".add_icon")).find("span")
						.addClass("icon");
				myLib.desktop.deskIcon.init();
		...
		function(event) {
			...
			//hxy 切换页面时图标位置调整
			if(i != '0' ){
				myLib.desktop.deskIcon.desktopMove($innerPanel, $deskIcon,
						$navTab, 500, desktopWidth + lBarWidth - 73, i);
			}else{
				myLib.desktop.deskIcon.desktopMove($innerPanel, $deskIcon,
						$navTab, 500, desktopWidth + lBarWidth, i);						
			}
//			myLib.desktop.deskIcon.desktopMove($innerPanel,$deskIcon,$navTab,500,desktopWidth,i);
...
myLib.desktop.wallpaper = {
	init : function(imgUrl) {
		...
		var myData = myLib.desktop.getMydata(), winWh = myData.winWh, wallpaper = myData.panel.wallpaper, _this = this;

		if (imgUrl !== null) {
			wallpaper.html("<img src='" + imgUrl + "?v=1.0.2'></img>");
		...
		// 如果窗口大小改变，更新背景布局大小
		window.onresize = function() {//窗口大小被改变时自动调用自己
			_this.init(imgUrl);
		};
	}
};
...
myLib.desktop.deskIcon = {
	...
	arrangeIcons : function(desktop) {
		var myData = myLib.desktop.getMydata(), winWh = myData.winWh, $navBar = myData.panel.navBar, navBarHeight = $navBar
				.outerHeight()
		// 计算一共有多少图标
		...
		var gg = 12;
		var pageWidths = window.innerWidth;
		
		if(pageWidths > 1900){
			gg = 80;
		}
		
		var gH = 120 + gg;// 一个图标总高度，包括上下margin
		var gW = 120 + gg;// 图标总宽度,包括左右margin
	...
	upDesktop : function($deskIcon, $deskIconBlock, $innerPanel, $deskIconNum,
			navBarHeight, lBarWidth) {
		var myData = myLib.desktop.getMydata(), 
					 winWh = myData.winWh, 
					 w = winWh['w'] - lBarWidth, 
		...
		$deskIcon.css({
			"width" : w + "px",
			"height" : h + "px",
			"margin-right" : lBarWidth + "px"  //hxy刷新页面时计算边距： -73
		});
		$deskIconBlock.css({
			"width" : w+ "px",
			"height" : h+ "px",
	...
	desktopMove : function($innerPanel, $deskIcon, $navTab, dates, moveDx,
			nextIndex) {
		$innerPanel.stop().animate(
				...
				dates,
				function() {
					$deskIcon.removeClass("currDesktop").eq(nextIndex)
							.addClass("currDesktop");

					$navTab.removeClass("currTab").eq(nextIndex).addClass(
							"currTab");
				});
	},
	init : function(iconData) {
		var myData = myLib.desktop.getMydata(),//获取body中的元素数据
			winWh = myData.winWh, //或者窗口的宽高
			$deskIconBlock = myData.panel.desktopPanel['_this'],//destop面板，div对象
			$innerPanel = myData.panel.desktopPanel.innerPanel, //内部面板，div对象
			$deskIcon = myData.panel.desktopPanel['deskIcon'], //ul对象
			$deskIconNum = $deskIcon.size(), //li个数
			$navBar = myData.panel.navBar, //顶部切换桌面按键
			navBarHeight = $navBar.outerHeight(), //元素外部高度
			$navTab = $navBar.find("a"), //找到顶部切换页面的a标签
			lBarWidth = myData.panel.lrBar["_this"].outerWidth(), //左侧边框的外部高度
			_this = this;
		...
		$(window).wresize(
				function() {
					myLib.desktop.winWH();// 更新窗口大小数据
					_this.upDesktop($deskIcon, $deskIconBlock, $innerPanel,
							$deskIconNum, navBarHeight, lBarWidth);
				});
		// 附加data数据
		myLib.desktop.iconDataInit(iconData);
		...
		$innerPanel.draggable({
			...
			stop : function(event, ui) {
				...
				dxEnd = event.pageX;
				var timeCha = timeEnd - timeStart, dxCha = dxEnd
						- dxStart, currDesktop = $(this).find(
						"ul.currDesktop"), deskIndex = $deskIcon
						.index(currDesktop), moveDx = $deskIcon.width()
						+ lBarWidth, dates = 1000 + timeCha;

				// 左移
				if (dxCha < -150 && deskIndex < 3) {
					_this.desktopMove($(this), $deskIcon, $navTab,
							dates, moveDx, deskIndex + 1);
					// 右移
		...
		.click(function() {
			var data = $(this).data("iconData"), id = this.id;
			var iconOpenType = getCookie("iconOpen");
//			console.info(iconOpenType == '0');
			if(iconOpenType == '0'){
				myLib.desktop.win.newWin({
					WindowTitle : data.title,
					iframSrc : data.url,
		...
		}).droppable(
				{
					...
					drop : function(event, ui) {
						ui.draggable.unbind("dblclick")
								.addClass("desktop_icon").insertBefore($(this))
		...
		var data = [ /*[ {
			text : "首页",
			func : function() {
				window.open("/seismis/dashboard/mapindex","_blank");
			}
		} ],*/[ {
			text : "刷        新",
			func : function() {
			...
			text : "退出系统",
			func : function() {
					window.open("/seismis/logout","_self");
			}
		} ]*/, [ {
			text : "关于seismis",
			func : function() {
